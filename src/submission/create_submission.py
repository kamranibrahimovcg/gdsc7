...
from src.submission.crews.BreakingRead_agent import BreakingReadPIRLS
...
def create_submission(call_id: str) -> Submission:
    ...
    llm = ChatBedrockWrapper(model_id=MODEL_ID, model_kwargs={'temperature': 0}, call_id=call_id)
    crew = BreakingReadPIRLS(llm)
    return crew
